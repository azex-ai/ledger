...
describe("use-withdrawals", () => {
  ...
  test("useWithdrawals surfaces a failed classification lookup as an error, not a stuck-pending empty result (M2)", async () => {
    ...
    const { result } = renderHook(() => useWithdrawals({ holder: 7 }), {
      wrapper: wrapperWith(qc),
    });
