...
| `migrations.job.databaseUrlKey` | `""` | Secret key holding a DDL-capable URL (`ledger_owner`, once migration 049 transfers it ownership) for the migration Job, distinct from the serving pods' `ledger_app` URL. Empty = both use the same connection (today's default; see `docs/RUNBOOK.md` §9). |
