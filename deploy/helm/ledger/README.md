...
| `migrations.job.databaseUrlKey` | `""` | Secret key holding a DDL-capable URL (`ledger_owner`) for the migration Job, distinct from the serving pods' `ledger_app` URL. Empty = both use the same connection, which must be able to `CREATE ROLE` the first time it runs against a fresh database (see `docs/RUNBOOK.md` §9). |
