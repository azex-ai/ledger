...
func insertForgedJournal(t testing.TB, ctx context.Context, pool *pgxpool.Pool, f attestFixture, idemKey string) int64 {
	...
	require.NoError(t, pool.QueryRow(ctx, `
		...
		VALUES ($1, $2, 1, 1, '{}'::jsonb, 0, 'attest-test', 0, now(), gen_random_uuid(), ''::bytea, ''::bytea, '')
		RETURNING id
...
func TestAttestationService_LateArrivingEntryIsEventuallyCoveredExactlyOnce(t *testing.T) {
	...
	journalID := insertForgedJournal(t, ctx, pool, f, postgrestest.UniqueKey("attest-late"))

	// txLate gets the LOWER id (inserted first) but stays open (uncommitted).
	txLate, err := pool.Begin(ctx)
	require.NoError(t, err)
	entryLate := insertEntryInTx(t, ctx, txLate, f, journalID, 9101, "debit")

	// txEarly gets the HIGHER id but commits immediately.
	txEarly, err := pool.Begin(ctx)
	require.NoError(t, err)
	entryEarly := insertEntryInTx(t, ctx, txEarly, f, journalID, 9102, "credit")
	require.NoError(t, txEarly.Commit(ctx))
	require.Greater(t, entryEarly, entryLate, "test setup: entryEarly must have the higher id despite committing first")

	// Batch 1: only entryEarly is visible (entryLate's transaction has not
	// committed yet) -- this is the batch that, under an id-range design,
	// would close out "as of" entryEarly's id and never look back.
	attested1, seq1, err := svc.RunAttestBatch(ctx, 100)
	require.NoError(t, err)
	require.Equal(t, 1, attested1)

	seqEarly, countEarly := entryAttestationSeq(t, ctx, pool, entryEarly)
	require.Equal(t, 1, countEarly)
	require.Equal(t, seq1, seqEarly)

	seqLateBefore, countLateBefore := entryAttestationSeq(t, ctx, pool, entryLate)
	require.Zero(t, countLateBefore, "entryLate must not be covered before it is even committed")
	_ = seqLateBefore

	// Now the late entry commits -- it becomes visible with an id LOWER
	// than something already covered by seq1.
	require.NoError(t, txLate.Commit(ctx))

	// Batch 2 must find and cover it.
	attested2, seq2, err := svc.RunAttestBatch(ctx, 100)
	require.NoError(t, err)
	require.Equal(t, 1, attested2)
	require.NotEqual(t, seq1, seq2)

	seqLate, countLate := entryAttestationSeq(t, ctx, pool, entryLate)
	require.Equal(t, 1, countLate, "the late entry must be covered exactly once")
	require.Equal(t, seq2, seqLate)

	// Re-confirm entryEarly was not touched or double-covered by batch 2.
	_, countEarlyAfter := entryAttestationSeq(t, ctx, pool, entryEarly)
	require.Equal(t, 1, countEarlyAfter, "the earlier entry must still be covered exactly once, not re-covered")
}
...
func TestAttestationService_ChainLinksPrevRoot(t *testing.T) {
	...
	// Post one entry, then seq 2 covers it.
	journalID := insertForgedJournal(t, ctx, pool, f, postgrestest.UniqueKey("attest-chain"))
	tx, err := pool.Begin(ctx)
	require.NoError(t, err)
	insertEntryInTx(t, ctx, tx, f, journalID, 9201, "debit")
...
func TestNaiveIDRangeWatermark_WouldMissTheLateEntry(t *testing.T) {
	...
	journalID := insertForgedJournal(t, ctx, pool, f, postgrestest.UniqueKey("attest-naive"))

	txLate, err := pool.Begin(ctx)
	require.NoError(t, err)
	entryLate := insertEntryInTx(t, ctx, txLate, f, journalID, 9301, "debit")

	txEarly, err := pool.Begin(ctx)
	require.NoError(t, err)
	entryEarly := insertEntryInTx(t, ctx, txEarly, f, journalID, 9302, "credit")
	require.NoError(t, txEarly.Commit(ctx))
	...
	require.Equal(t, entryEarly, watermark, "batch 1's watermark closes out at entryEarly (entryLate is not yet visible)")
