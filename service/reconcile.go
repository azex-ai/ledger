...
// docs/RUNBOOK.md section 18 is the procedure.
	...
	for _, r := range rows {
		...
		case "over_reversed":
			...
			result.Findings = append(result.Findings, core.Finding{
				...
				Detail: "violation=over_reversed; see docs/RUNBOOK.md section 18 for the query that lists every journal linked to this one",
			})
		default:
			...
			result.Findings = append(result.Findings, core.Finding{
				...
				Detail: fmt.Sprintf("violation=%s amount=%s; see docs/RUNBOOK.md section 18", r.Violation, r.ReversedAmount.String()),
			})
		}
	}
