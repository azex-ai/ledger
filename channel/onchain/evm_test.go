...
func TestEVMAdapter_ParseCallback(t *testing.T) {
	...
	t.Run("valid JSON", func(t *testing.T) {
		...
		assert.Equal(t, "bk-42", payload.BookingUID)
	...
	t.Run("invalid amount", func(t *testing.T) {
		body := []byte(`{"tx_hash":"0x1","booking_uid":"bk-1","amount":"not-a-number","confirmations":1,"status":"pending"}`)
		header := http.Header{}
...
func TestEVMAdapter_ParseSighting(t *testing.T) {
	...
	t.Run("valid JSON", func(t *testing.T) {
		body := []byte(`{"chain_id":1,"tx_hash":"0xabc123","txlog_seq":2,"token":"0xusdt","from":"0xfrom","to":"0xTo1234","amount":"12.5","confirmations":6}`)
		header := http.Header{}
	...
	t.Run("invalid amount", func(t *testing.T) {
		body := []byte(`{"chain_id":1,"tx_hash":"0x1","txlog_seq":0,"token":"0xusdt","from":"0xfrom","to":"0xto","amount":"not-a-number","confirmations":1}`)
	...
	t.Run("fails Validate on missing required field", func(t *testing.T) {
		...
		body := []byte(`{"tx_hash":"0x1","txlog_seq":0,"token":"0xusdt","from":"0xfrom","to":"0xto","amount":"1","confirmations":1}`)
		_, err := a.ParseSighting(http.Header{}, body)
		require.Error(t, err)
