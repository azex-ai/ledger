...
> 2026-08-25 · Status: **CLOSED**（2026-08-26 收口）——八个 territory 全部交付，
> 报告与交叉分析在 `docs/audits/2026-08-25-financial-engineering/`，
> 8 条 Critical 已修复合入（处置表见该目录 README）。
>
> 本文档转为**方法记录**：territory 怎么切、纪律怎么定、以及三条从真实失效里长出来的
> 规则（§1.4「有测试不等于被验证」、§1.5「不要编造具体值」、§1.1 只读）。
> 下次做同类审计从这里起手，但 §2 的 territory 划分要重新按当时的代码想 ——
> 这次就漏了两个维度（可运维性、链上资金路径），由 lead 补审后产出 7 条含 3 条 Major。
