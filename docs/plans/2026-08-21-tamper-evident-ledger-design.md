...
> **起因**：一次真实的数据库泄露事件。要回答的问题不是「怎么防泄露」，而是
...
2. **app + KMS 同时失陷**。攻击者能调 KMS 就能给伪造记录签名。唯一缓解是留痕
   （KMS audit log 落在独立云账号）+ 限速，不是密码学保证。
...
- **必选载体**：独立云账号 + S3 / R2 **Object Lock（compliance mode）**，retention ≥ 账务保留期。
  ⚠️ 必须**独立账号 + 独立凭证**（`infra.md` 租户归属：不落个人账号）——
  同账号内的 object lock 挡不住该账号的 root。
- **可选加强**：公链锚定 32 bytes root（不上明细，成本可忽略）。port 留口，本期不实现。
