...
| 7.14 复审 M-1 / M-3 / M-5 + hold 项方案 (A) + `ConfirmPending` 的 pending 余额闸（`pending_store.go:302`，同 M-1 读 checkpoint+delta；w3-holds 兄弟扫描补出的第三个消费点）（待 Aaron） | `enforce_min_balance` 读 checkpoint（透支闸可被 INSERT 破）；`event_uid` 冒领后 booking 永久锁死无解除路径；`Migrate` 提权窗口是角色级，迁移期间应用连接池 owner-equivalent | 本波不动代码。推荐：M-1 明文声明信任边界 + 依赖 reconcile check #2 检测（热路径 entries-only 重算代价高）或引入签名 checkpoint；M-3 owner-only 解除路径；M-5 文档要求迁移凭证 ≠ 应用凭证，且迁移窗口内应用不得在线；hold 项若保守口径的产品代价不可接受，则做 (A)：ReserverStore 持 Attestor 给 receipt/leg 签名，闸在事务外验签 | Aaron |
| 7.15 复审 M-3 / M-5 的落地（lead 拍板，2026-09-03） | `event_uid` 冒领后 booking 永久锁死无解除路径；`Migrate` 提权窗口角色级 | **M-3 做 (A)**：owner-only 的 `ledger_unlink_event_journal(event_uid)`（SECURITY DEFINER，只 GRANT ledger_owner），写 `config_table_changes`，RUNBOOK 处置流程，I-51 补注。**M-5 只做文档层**：`Migrate` 凭证 ≠ 应用凭证且迁移期间不得有其它会话；examples 拆 `MIGRATE_DATABASE_URL` / `DATABASE_URL`；RUNBOOK 自相矛盾句删除；静态门禁「example 不用同一 URL 喂 Migrate 与 pgxpool」。**M-5 机制层（owner 连接交给 migrate 驱动，窗口从角色级降到连接级；顺带修 `elevateToLedgerOwner` 擅改 SET-only 成员资格为 INHERIT 并整行 REVOKE）待 Aaron**——先补一次实跑定级（非 superuser runner + 迁移中途同角色另一连接 `DROP TRIGGER`） | W3（w3-fixes 第二轮，worktree w3-fixes-2，migration 027） |
| 7.16 M-5 机制层（lead 拍板，2026-09-03） | w3-fixes 实跑：单凭证部署下应用连接在迁移进行中 `DROP TRIGGER` 成功、`Migrate` 返回 nil、I-22 守卫永久消失——**Critical**，文档层依赖运维照做 | 在「全量修复」授权下**本波做**（Aaron 可否决）：owner 连接交给 migrate 驱动（`migrate.NewWithInstance` + 以 ledger_owner 身份认证的 `*sql.DB` 或 `SET ROLE` 的专用连接），窗口从角色级降到连接级；不再改角色图；`elevateToLedgerOwner` 擅改 SET-only 成员资格为 INHERIT 并整行 REVOKE 的副作用一并消除。pin = 复审员的攻击探针（迁移中途同凭证第二连接 `DROP TRIGGER` 必须 42501） | W3（w3-migrate，worktree w3-migrate）。**2026-09-03 修订**：worker 实测 001 结尾把 runner 的 SET 成员资格整行撤掉，`SET ROLE` 不可直接用。改为三层：① `GRANT ledger_owner TO runner WITH SET TRUE, INHERIT FALSE`，只在一条专用连接 `SET ROLE`，结束 RESET + REVOKE；② **同角色会话守卫**：GRANT 前后查 `pg_stat_activity`，同凭证除本进程两条连接外若有任何后端在线 → 拒绝迁移（fail-closed，文案要求分开凭证或停应用）；③ superuser / ledger_owner 本人跳过 |
| 7.17 M-5 机制层的三条尾巴（lead 拍板，2026-09-03） | ① 同角色会话守卫是否豁免「操作方已授予 SET」的路径；② 共享集群上 `ledger_app` 已带 SUPERUSER 时 007 改为 fail-loud；③ 迁移中途新开的同凭证连接仍可显式 `SET ROLE`（守卫只看 run 开始时的会话） | ① **不豁免**（两种情况对第二条连接的暴露相同，豁免会留「半夜被拒→自己 GRANT SET→守卫永久消失」的旁路）；② 接受，进 BREAKING；③ **接受为已知残留**，以「钉住实测行为」的 pin + I-22 / RUNBOOK / godoc 注记存在；不采用 `CONNECTION LIMIT 0` 方案（崩溃残留连不上的凭证 = 共享凭证部署停机） | 已合入（`w3-migrate-residual`） |
| 7.18 §7.14 信任边界项（lead 按原则拍板，Aaron 2026-09-03 授权） | `enforce_min_balance` / `ConfirmPending` 读 checkpoint+delta；hold 项是否做签名 receipt | **`enforce_min_balance`：保持 checkpoint+delta。** 它是应用凭证信任边界内的业务规则闸（策略表本身 `ledger_app` 可写，硬化余额项买不到东西，且它只减余额不造钱）；在 I-17 明文声明「不属于防篡改提现路径，出金只认 `RequireVerifiedBalance`」。**`ConfirmPending`：改 entries-only 重算**（与 I-49 的 E 同形，锁内纯 SQL）——它在造钱路径上：伪造 checkpoint → 确认不存在的 pending → 应用签名的 journal 让 E 认账 → 验签闸放行出金。**hold 项：做方案 (A)**——`ReserverStore` 持 Attestor，Settle/SettlePartial/Release/Finalize 写 receipt/leg 时签名（覆盖 reservation uid、操作、金额、序号），闸在事务外验签算出各预留的可信已解除额，锁内用「预验证过的解除额」；未配 Attestor 或任一签名不过 → 该预留退回 §7.12 的完全保守口径。原则依据：本威胁模型下签名是唯一能区分应用与攻击者的信号（设计文档 §0） | Wave 4：w4-pending-gate、w4-signed-holds |
| 7.19 §7 全部口径确认（lead，2026-09-03） | 7.1–7.17 | 逐条对照 `financial.md`（decimal / append-only / 幂等 / 事务内无外部调用）、`working-agreements.md` §3（fail-closed）、§5（结构强制）、防篡改设计 §0（checkpoint 不可信、签名是唯一可信信号）：**全部维持**。7.12 由 7.18 演进（签名 receipt 为主，保守口径为退路） | — |
| 7.20 `ConfirmPending` 的 V 项（lead 拍板，2026-09-03） | w4-pending-gate 实测：往 pending 维度伪造**未签名** entries，`ConfirmPending` 用应用真签名的 journal 把它搬进 main_wallet，I-49 的 V 与 E 都认账——`ConfirmPending` 是唯一的洗白点 | 配置了 Attestor 时，`ConfirmPending` 在开事务前对 pending 维度全部 contributing journals 验签，任一失败 → `ErrUnauthorizedJournal`（UNDEFINED 语义，不排除坏的求剩余）；tx 模式 + Attestor → fail-closed `ErrInvalidInput`；未配 Attestor → E-only 并在 I-64 写明不具防篡改性。`CancelPending` 只减不造钱，不验 | Wave 4：w4-pending-gate（追加） |
