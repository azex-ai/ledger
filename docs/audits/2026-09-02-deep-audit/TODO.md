...
> ## ⚠️ 本文件已不是待办清单 —— 见「处置状态」
>
> 2026-09-02/03 的三波整改（Wave 1 六条 Critical、Wave 2 八条 Major/Minor 主体、Wave 3 lead 收口）
> 已把本清单的 207 条独立条目全部处置：修复合入 main、明确记为不修（附结论）、或转入文末「Lead 追加」表的
> W3 后续项。**下面按任务归属排列的条目是整改开工时的原始清单，保留作为论证与定位索引，不再代表「还剩什么」。**
> 还剩什么看审计 `README.md` 的处置表、`lead-verification.md` 的合并记录（每条合入都带 lead 的 mutation 证据）、
> 以及本文件文末「Lead 追加」表中标 W3 后续 的行。
>
> 行号核对到 `fd87cae`；整改后行号已偏移，以符号名为准。
...
| I-M1 (D-ops) | `core.Metrics` 的 `BalanceDrift` / `NegativeBalanceDetected` / `ReconcileGap` / `ReservedAmount` 四个方法的 `currencyID int64` 参数改为 `currencyUID string`（H-M9：core 接口不得暴露内部 id） | 手写（非嵌入 `NoopMetrics`）的 `core.Metrics` 实现需改这四个方法签名；scrape 这四个 Prometheus 指标的 `currency_id` label 改名为 `currency_uid` |
| I-M1 / I-M8 / I-M10 / B-m10 (D-ops) | `core.Metrics` 从 32 个方法增至 41 个：新增 `JobTickCompleted` / `JobTickFailed` / `JobTickSkippedLocked` / `JobPanicked` / `StuckRollups` / `PendingEvents`（声明未发射，见下）/ `AttestationBatchResult` / `AnchorPublishResult` / `AnchorLagSeqs` | 手写 `core.Metrics` 实现需新增全部 9 个方法（嵌入 `NoopMetrics` 的实现不受影响）。`LedgerStore` / `ReserverStore` / `BookingStore` 新增 `WithMetrics(core.Metrics)` 链式方法，默认 `core.NopMetrics()`，`ledger.New` 自动接线 |
| I-N12 (D-ops) | `delivery.NewLocalDispatcher` 新增第三个必填参数 `metrics core.Metrics`（nil 视为 `core.NopMetrics()`） | 直接构造 `LocalDispatcher`（而非经 `ledger.Service.Worker()`/`service.Worker.Subscribe` 间接构造）的调用点需补第三个参数 |
...
| w4-pending-gate（破坏性变更） | `postgres.NewPendingStore` 加第四个参数 `verifiedBalance core.VerifiedBalanceReader`（契约 §7.20 / I-64）；且配置了 `core.Attestor` 时，`ConfirmPending` 在 `RunInTx` 内被调用改为返回 `core.ErrInvalidInput`（不再降级为无闸执行） | 走 `ledger.New` 的消费方**无动作**（`WithAttestor` 配了就自动注入，没配传 nil）。自建组合根：补第四个参数，或传 `nil` 保持原 entries-only 行为。**在 `RunInTx` 回调里 confirm 的消费方必须把 confirm 移到事务之外**（先 confirm 再开事务，错误文案已写明）；`CancelPending` 不受影响、仍可在 `RunInTx` 内复合；未配 `Attestor` 的部署完全不受影响。详见 `docs/BREAKING.md` 与 CHANGELOG |
| W5 后续（lead 追加，2026-09-04） | **migration 027 两个 set-once 守卫的门谓词不 COALESCE**：`current_setting(..., true)` 未设时返回 NULL，`IF NOT <null> THEN RAISE` 不会 raise——单看谓词是 fail-open 形态。目前被前置的 `NEW.journal_id IS NULL AND ...` 合取挡住（攻击态该合取为 false），029 的 `ledger_chain_cursor_rewind_is_authorized` 已按 COALESCE 写法并在头注里点名。源：w5-insert 在 029 头注「reported separately rather than edited here」 | W5 后续：给 027 两个谓词补 COALESCE（同 029 写法）+ 一条「未设 GUC 时 owner 直接 UPDATE 必拒」的 pin；不阻塞 Wave 5 合并 |
| W5 后续（worker 交接，2026-09-04） | **w5-onchain-ops 留下 4 条**：① money-out C-2 建议 1 的围栏（`AutoCreditCeiling>0` 但无 `DepositConfirmer` 是否拒启动——现已做更强的主源核对，是否还要待拍板）；② `service/reconcile.go` 对 skipped check 发 `ReconcileCheckResult(name,false)`（onchain-ops M-5 第 2 项）；③ `resolveSweepCurrency` 的 token 未归一大小写（会让该 (chain,token) 每 tick 失败）；④ DR §5 配方 / CAPACITY 链上覆盖（`docs/api.md` 的 dead-letter 两个端点章节**已补**：§17 记 `GET /deposits/dead-letters` 与 `POST /deposits/dead-letters/{uid}/replay`，`f1d3992`）。**w5-gates 留下 2 条**：P-4 `docs/frontend.md` 无一致性门禁（`readme_docs_consistency_test.go` 的对应物）；P-5 `anchortest.Check` 未探的两维（并发 `Publish` 同 seq、网络分区下 `Head` 返回旧值而非报错）。**w5-money-misc 观察**：`cmd/ledger-cli` 的 `TestCmdConfigHistory_TableFilter_ReturnsTheRealTriggerWrittenRow` 全套并行时一次 61s 红、单跑绿——疑容器/锁竞争 flake | W5 后续，不阻塞发版评估；③ 与 ② 建议下一波先做（钱路径与信号路径） |
| R3 复核残余（lead 追加，2026-09-04） | **install-roles**：m2（RUNBOOK 仍把 `ledger_owner` 本人列为可用迁移凭证而未注明「仅对已安装的库」；`ledger_app` 报错的诊断语假定「同集群已有 ledger 库」，最常见真因是 MIGRATE_DATABASE_URL 指错）、m3（bootstrap 凭证对三个角色都持 ADMIN OPTION，001 头注与 RUNBOOK 只提 ledger_owner，退役凭证需 superuser 收回三条）、m7（无运维侧 ACL/ownership/trigger 矩阵自检入口）。**consumer**：F-m6 `make db` 开箱失败、F-m8 `examples/tx-compose` 以 ledger_app 跑死在 CREATE TABLE、F-m9 复用 anchordev 目录跨重建库报 TAMPERED 与真攻击不可区分、F-m10 store getter 命名四种风格、F-m13/m14 `tolerance` 缺失文案与 `?limit=abc` 静默回落、F-m15 README 未提 `Subscribe` 返回 error、N-2 全新安装首轮 `RunAttestBatch` 因空 anchor 的 `observed_head` NOT NULL 记 ERROR 且方法返回 nil、F-M1 残余 3（HTTP 面无端点可挂 lifecycle）、F-M3 残余（无接口读闸视角可用额）。**money-out**：m-1 升 CONFIRMED（`rollup_queue` append `claimed_until='2099'` 堵 `RebuildCheckpoint`，`unauditedWrites` 对 `rollup_queue/INSERT` 的豁免理由对此形态不成立）、m-2（`journal_entries.effective_at` 可与父 journal 不一致，绕过 `period_close_violations`）、m-3（一条 entry append 让 holder 的 gated 提现永久 UNDEFINED，无 remediation API）、m-4（`ledger_app` 仍可读 `webhook_subscribers.secret`，D-M6 contract 阶段待做）、N-3 结构性修法（discard 记入 `anchor_observations` 让 VerifyLedger 区分「已记录的 discard」与回滚） | W5 后续，不阻塞发版评估；m-1 与 money-out m-2 建议下一波先做 |
| R3 复核残余 · onchain-ops（lead 追加，2026-09-04） | Minor-5 DR §5 的 `jq -e '.overall_passed == true'` 未同步 RUNBOOK §1 的三重断言（DR 给的是弱判据）；Minor-6 PITR 会丢 T 之后的 `deposit_reorgs`，DR 未提、§5 清单不查未结异常队列；Minor-7 CAPACITY 对链上路径零覆盖；Minor-9 RUNBOOK:473 附近 ```bash 块内容是 SQL；Minor-11 `ReservedAmount`/`PendingEvents` 死指标仍注册且 `PendingEvents` 文档陈述假事实；M-6 残余：resolve 后 booking 仍在 `reorgRecheckWindow` 内时每 tick 重发 `DepositReorgDetected`（有界约 500 块，§12 未说明）；「其他观察」5 条未变（`priorTxHash=""`、`resolveSweepCurrency` 大小写、`packBatchSweep` 无 `IsHexAddress`、`HasOpenReorg` 死代码、CLI usage 与实现不符） | W5 后续 |
| W5 后续 · 032 兄弟扫描剩项（w5-onchain-ops 交接，2026-09-04） | ① sweep booking 的身份 `(chain_id, token, nonce)` 在 metadata 里、无 DB 唯一约束（sweep 从不 post journal，复制不能铸币，但会让 `findInFlightSweep` 在多行里挑一行）；② `reservation_operation_receipts` / `reservation_settlement_legs` 只有 `idempotency_key UNIQUE`，第二张 discharge 凭据可追加——挡在它和真实 discharge 之间的只有 I-65 的签名要求；③ money-out N-3 结构性修法：discard 记进 `anchor_observations` 让 `VerifyLedger` 区分「operator 丢弃过一段」与「锚被投毒」（现为 RUNBOOK §21 时限纪律） | W5 后续；② 建议与 I-65 一起复核 |
| 术语统一残余（lead 追加，2026-09-04） | preset journal type 的 `Name` 数据仍为 "FX Sell Leg"/"FX Buy Leg"（`presets/fx.go`），`InstallTemplatePresets` 对已装库校验 Name 一致，改名需 expand → migrate → contract；`reservation_settlement_legs` / `SettlementLeg` 是预留结算凭据行的正式命名，与 journal entry 无关，保留 | W5 后续；若要改 `Name`，与下一次 preset 数据迁移一起做 |
