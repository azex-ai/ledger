...
| W5 后续（worker 交接，2026-09-04） | **w5-onchain-ops 留下 4 条**：① money-out C-2 建议 1 的围栏（`AutoCreditCeiling>0` 但无 `DepositConfirmer` 是否拒启动——现已做更强的主源核对，是否还要待拍板）；② `service/reconcile.go` 对 skipped check 发 `ReconcileCheckResult(name,false)`（onchain-ops M-5 第 2 项）；③ `resolveSweepCurrency` 的 token 未归一大小写（会让该 (chain,token) 每 tick 失败）；④ DR §5 配方 / CAPACITY 链上覆盖 / `docs/api.md` 的 dead-letter 两个端点章节。**w5-gates 留下 2 条**：P-4 `docs/frontend.md` 无一致性门禁（`readme_docs_consistency_test.go` 的对应物）；P-5 `anchortest.Check` 未探的两维（并发 `Publish` 同 seq、网络分区下 `Head` 返回旧值而非报错）。**w5-money-misc 观察**：`cmd/ledger-cli` 的 `TestCmdConfigHistory_TableFilter_ReturnsTheRealTriggerWrittenRow` 全套并行时一次 61s 红、单跑绿——疑容器/锁竞争 flake | W5 后续，不阻塞发版评估；③ 与 ② 建议下一波先做（钱路径与信号路径） |
