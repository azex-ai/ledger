...
| D-web | `c469ae8` 系列（4 commit，rebase 后） | BalancesPage 忽略 `isError` 的 mutation → `surfaces ErrorState when the trend request fails` 红；恢复后 ledger-react 11 文件全绿 | ff | 关 J 全部（J-12 部分闭合：a11y census 21→55，gate 改跨 skin 相对断言；J-23 CI 步骤移交）。openapi 变更后的 `schema.ts` 再生成由 lead 在 d-contract 合入后做 |
