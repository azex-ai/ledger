...
| W5-gates-2（5.18 R-4/R-3/R-2） | `894621c` `4674346` `968447c` | lead 三组 mutation：`Round` 去掉 `validateAmountIsRescalable` → `TestEveryAmountEntryPointRejectsAPathologicalAmount` 红；删 `go-release.yml` 的 anchors/r2 govulncheck 步 → `TestReleaseSubmoduleJobsRunTheSameChecksAsGoVerify` 红；`appendOnlyGuardFunctions` 缩回单名 → `TestAppendOnlyGuards_EveryTriggerRefusesItsMutation` floor 红 | merge `9bb2acd`（无冲突）；main 全量绿 | `Round`/`ConvertAt` 改返回 error（BREAKING）；worker 顺带发现负指数镜像形态 `1E-999999999` 同样不返回并一并封住（`MaxAmountWorkingFractionalDigits = 36`，保留 `scaleByFraction` 的 12 位 guard digits）；`FuzzAllocateFromStrings` 以 2s 时限为断言；census 从单函数名改为函数集合，floor 回到 25 |
